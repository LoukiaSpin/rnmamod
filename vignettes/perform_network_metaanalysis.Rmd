...
Bayesian analysis using the BUGS code of Dias and colleagues (2013). With only 
...
For instance, the output below presents the posterior distribution of the log 
odds ratio of comparisons with placebo, the reference intervention of the 
network.

```{r set-options, echo = FALSE, comment = NA}
options(width = 1000)
load("./output_data/baker_ln_results.RData")
res$EM_ref
```
...
(data_new <- nma.baker2009[, c(-10, -11, -12, -13)])
```

Using the minimum required arguments with 
`heter_prior = list("lognormal", -2.06, 0.438)`, `run_model` will run and 
provide results. For instance, for the log odds ratio of comparisons with
placebo: 

```{r echo = FALSE, comment = NA}
load("./output_data/em_ref_baker_ln_nomod.Rdata")
em_ref_baker_ln_nomod
```
