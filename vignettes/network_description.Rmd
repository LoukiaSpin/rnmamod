...
When missing participants have been reported for each intervention arm of at least one trial, `netplot` returns a table with the distribution of the proportion of missing participants **per intervention**:

```{r, echo = FALSE, fig.show = "hide"}
net$Table.interventions.Missing
```

and **per observed comparison** in the network:

```{r, echo = FALSE, fig.show = "hide"}
net$Table.comparisons.Missing
```

The users can export all tables in Excel 'xlsx' format at the working directory of their project by adding the argument `save.xls = TRUE` in the function.
