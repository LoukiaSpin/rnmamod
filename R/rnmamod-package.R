...
#'   - local evaluation of the consistency assumption using the fixed-effect or
#'   random-effects node-splitting approach (\code{\link{run_nodesplit}}) based
#'   on Dias et al., (2010), and van Valkenhoef et al., (2016);
#'   - global evaluation of the consistency assumption using the fixed-effect or
#'   random-effects unrelated mean effects model (\code{\link{run_ume}}) based
#'   on Dias et al., (2013a);
#'   - comprehensive sensitivity analysis for the impact of aggregate binary and
#'   continuous missing participant outcome data (\code{\link{run_sensitivity}})
...
#'   - tabulation of the R-hat (Gelman and Rubin, (1992)) for all monitored
...
#'   - heatmap on the proportion of missing participant outcome data across the
#'   network (\code{\link{heatmap_missing_network}}) and across the intervention
#'   arms of each trial in the dataset (\code{\link{heatmap_missing_dataset}});
#'   - league heatmap with the estimated and predicted summary effects of all
#'   possible pairwise comparisons in the network and integrated SUCRA or
...
#'   and Schwarzer, 2015);
#'   - rankograms with intergrated SUCRA values for each intervention in the
#'   network (\code{\link{rankosucra_plot}}) after performing network
...
#'   - tabulation of the estimated regression coefficient(s), the estimated and
#'   predicted summary effects, measures of model fit and estimated
#'   between-trial variance before and after adjusting for a trial-specific
#'   covariate (\code{\link{metareg_plot}}), and visualisation of the summary
#'   effects and SUCRA values from both models
...
#'   estimated between-trial variance after each split node, and visualisation
#'   of these results (\code{\link{nodesplit_plot}});
#'   - tabulation of the estimated summary effects of all comparisons observed
#'   in the network, measures of model fit and estimated between-trial variance
#'   under the unrelated mean effects model and network meta-analysis, as well
...
#'   results (\code{\link{series_meta_plot}});
#'   - calculation and of the robustness index for all possible comparisons in
#'   the network (\code{\link{robustness_index}} and
#'   \code{\link{heatmap_robustness}}, respectively) (Spineli et al., (2021));
#'   - enhanced balloon plot with the summary effects and between-trial variance
#'   for a selected pairwise comparison under several scenarions about the
#'   missing mechanisms (\code{\link{balloon_plot}}) (Spineli et al., (2021));
