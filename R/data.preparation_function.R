...
#'
#' @description This function prepares the dataset in the proper format for R2jags. \code{data_preparation} is found in the \code{run_model} function; thus, the arguments are as specified in the latter.
#'
#' @param data A data-frame of the one-trial-per-row format containing arm-level data of each trial. This format is widely used for BUGS models. See 'Format' in the \code{run_model} function.
...
#'
#' @return A list of data-frames on the following elements to be passed to \code{\link{run_model}}:
#' \tabular{ll}{
#'  \code{m} \tab The number of missing participant outcome data in each arm of every trial (see 'Details' in the \code{run_model} function).\cr
#'  \tab \cr
...
#'  \tab \cr
#'  \code{I} \tab The pseudo-data-frame \code{I} (see 'Details' in the \code{run_model} function).\cr
#'  \tab \cr
...
#' }
#'
#' @details \code{data_preparation} prepares the data for the Bayesian analysis. The data preparation includes the following actions. First, \code{data_preparation} checks
#'   whether the element \strong{m} exists in \code{data}. If this element is missing, \code{data_preparation} creates a pseudo-data-frame for \code{m} that has the zero value for the observed trial-arms, and \code{NA} for the unobserved trial-arms,
...
#'   when MOD have not been extracted for any trial of the dataset. Second, \code{data_preparation} sorts the interventions across the arms of each trial in an ascending order and correspondingly sorts within each trial the remaining elements in \code{data}
#'   (see also 'Format' in \code{run_model}). Since all Bayesian models in the package consider the first column in \strong{t} to include the control arm for every trial, this sorting ensures that interventions with a lower identifier are consistently treated as the control arm in each trial. This case is relevant in
#'   non-star-shaped networks.
#'
#' @author {Loukia M. Spineli}
#'
#' @seealso \href{https://CRAN.R-project.org/package=R2jags}{R2jags}, \code{\link{run_model}}
#'
#' @export
