...
#'   This is necessary to ensure transitivity in the assumed values for the missingness parameter across the comparisons in the network (Spineli, 2019). When one numeric value is considered, the same missingness scenario is applied to all interventions in the dataset.
#'
#'   Currently, there are no empirically-based prior distributions for the informative missingness parameters. The users may refer to Mavridis et al. (2015) and Spineli (2019) to determine \code{mean.misspar} for an informative missingness mechanism.
#'
