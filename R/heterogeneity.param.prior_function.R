...
#'
#' @description
#'   Generates the prior distribution for the between-trial standard deviation
#'   depending on the selected effect measure.
#'   \code{heterogeneity_param_prior} is found in the \code{run_model} function;
...
#'   is ignored in the \code{run_model} function.
#'
...
heterogeneity_param_prior <- function(measure, model, heter_prior) {
  ...
  if (model == "RE" &
    ...
    as.numeric(c(heter_prior[[2]], heter_prior[[3]], 3))
             ...
             measure != "OR" &
             heter_prior[[1]] == "lognormal") {
    stop("Not the proper prior distribution for continuous outcome", call. = F)
  } else if (model == "RE" &
             measure == "SMD" &
             heter_prior[[1]] == "logt") {
    as.numeric(c(heter_prior[[2]], heter_prior[[3]], 3))
