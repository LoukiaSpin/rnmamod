...
#'
#' @export
league.heatmap <- function(full, cov.value = NULL, drug.names){
  ...
  if (is.null(full$beta.all)) {
    ...
    par.mean <- full$EM[, 1] + full$beta.all[, 1]*covar
  ...
  caption <- if (!is.null(full$beta.all) & length(unique(full$covariate)) > 2) {
    paste("Posterior mean (95% credible interval) for", cov.value[2], cov.value[1])
  } else if (!is.null(full$beta.all) & length(unique(full$covariate)) < 3) {
    paste("Posterior mean (95% credible interval) for", cov.value[2])
