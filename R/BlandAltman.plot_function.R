...
BlandAltman.plot <- function(group1, group2, colour){
  ...
  lowerLoA <- bias - 1.96*stats::sd(difference)
  upperLoA <- bias + 1.96*stats::sd(difference)

  ggplot2::ggplot(data, ggplot2::aes(x = data[, 1], y = data[, 2])) +
    ggplot2::geom_point(size = 2, colour = colour) +
    ggplot2::geom_hline(yintercept = 0, linetype = 2) +
    ggplot2::geom_hline(yintercept = bias, linetype = 1) +
    ggplot2:: geom_hline(yintercept = lowerLoA, linetype = 1) +
    ggplot2::geom_hline(yintercept = upperLoA, linetype = 1) +
    ggplot2::geom_text(ggplot2::aes(x = data[, 1], y = data[, 2], label = data[, 3]), color = "black", fontface = "bold",
    ...
    ggplot2::labs(x = paste("Average posterior mean deviance contribution"),
    y = paste("Difference in posterior mean deviance contribution")) +
    ggplot2::theme(axis.text = ggplot2::element_text(size = 12), axis.text.x = ggplot2::element_text(size = 12),
            axis.text.y = ggplot2::element_text(size = 12), axis.title = ggplot2::element_text(size = 12))
