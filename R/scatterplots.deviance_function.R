...
scatterplots.dev <- function(full, ume, colour) {
  ...
  ggplot(data = prepare.dev, aes(x = full, y = ume)) +
    ...
    scale_x_continuous(limits = c(min(prepare.dev$full, prepare.dev$ume), max(prepare.dev$full, prepare.dev$ume)), labels = scaleFUN) +
    scale_y_continuous(limits = c(min(prepare.dev$full, prepare.dev$ume), max(prepare.dev$full, prepare.dev$ume)), labels = scaleFUN) +
    theme_classic() +
